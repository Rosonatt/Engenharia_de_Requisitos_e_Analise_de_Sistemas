...
def devolver_livro(titulo:str, nome_usuario:str):
    """
    Permite que um usuário devolva um livro que ele tenha emprestado.

    Parâmetros:
    titulo (str): Título do livro a ser devolvido.
    nome_usuario (str): Nome do usuário que está devolvendo o livro.
    """
    ...
    if titulo not in usuarios[nome_usuario]:
        print(f'O usuário "{nome_usuario}" não tem o livro "{titulo}" emprestado.')
        return

    for livro in livros:  # Itera sobre os livros para encontrar o livro desejado
        
        if livro['titulo'].lower() == titulo.lower():  # Compara títulos de forma insensível a maiúsculas
            livro['disponivel'] = True  # Marca o livro como disponível
            usuarios[nome_usuario].remove(titulo)  # Remove o livro da lista de livros emprestados do usuário
            # Registra a atividade de devolução
            registrar_atividade("Devolução", titulo, nome_usuario)
            print(f'Livro "{titulo}" devolvido por {nome_usuario} com sucesso!')
            return
...
def menu_usuario(nome_usuario):
    ...
    while True:
        ...
        print("2. Procurar Livro (Título, Autor ou Gênero)")
        print("3. Emprestar Livro")
        print("4. Devolver Livro")  # Adiciona a opção de devolver livro
        print("5. Sair")
        ...
        if opcao_usuario == '1':
            ...
            devolver_livro(titulo, nome_usuario)  # Chama a função de devolução de livro
